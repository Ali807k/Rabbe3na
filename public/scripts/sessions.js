...
async function getJalsah() {
	...
	.then((data) => {
		showJalsaat([data], "private")
	})
	.catch((error) => {
		console.error("Error fetching jalsaat:", error)
		Swal.fire('Failed!', 'Failed to find the jalsah!');
	});
}

function showJalsaat(jalsaat, gameAccess) {
	...
	jalsaat
	.filter((jalsah) => jalsah.gameAccess === gameAccess)
	.forEach((jalsah) => {
					...
					<div class="jalsah-container">
						...
						<div class="jalsah-info-container">
							...
							<div class="jalsah-info">
								<span class="jalsah-location">${jalsah.shortLocation}</span>
								<span class="jalsah-time">${jalsah.time}</span>
							</div>
						</div>
					</div>
				</a>
...
async function createJalsah(event) {
	...
	const shortLocation = document.getElementById("shortAddress").value.trim();
	...
	.then((data) => {
		...
		data.forEach(jalsah => {
			...
			jalsahElement.addEventListener("click", (e) => {
				...
				`/jalsah?id=${jalsah._id}
				&desc=${encodeURIComponent(jalsah.description)}
				&time=${encodeURIComponent(jalsah.time)}`;
			})
			sidebar.appendChild(jalsahElement);
		})
	})
	.catch((error) => console.error("Error fetching jalsaat:", error));
}
...
document.addEventListener("DOMContentLoaded", getUserJalsaat);
document.getElementById("createButton").addEventListener("click", createJalsah);
document.getElementById("search-button").addEventListener("click", getJalsah)
